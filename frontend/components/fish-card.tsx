...
const RARITY_COLORS = {
  ...
  legendary: "bg-orange-100 text-orange-900 dark:bg-orange-900 dark:text-orange-100",
}
...
const RARITY_HOVER_BACKGROUNDS = {
  [FishRarity.COMMON]: "group-hover:from-gray-100 group-hover:to-slate-100 dark:group-hover:from-gray-900 dark:group-hover:to-slate-900",
  [FishRarity.RARE]: "group-hover:from-blue-100 group-hover:to-cyan-100 dark:group-hover:from-blue-900 dark:group-hover:to-cyan-900",
  [FishRarity.EPIC]: "group-hover:from-purple-100 group-hover:to-violet-100 dark:group-hover:from-purple-900 dark:group-hover:to-violet-900",
  [FishRarity.LEGENDARY]: "group-hover:from-orange-100 group-hover:to-amber-100 dark:group-hover:from-orange-900 dark:group-hover:to-amber-900",
}
    ...
    >
      <div className={`h-48 flex items-center justify-center relative overflow-hidden transition-all duration-500 ease-out group-hover:brightness-110 bg-gradient-to-b ${RARITY_BACKGROUNDS[rarity]} ${RARITY_HOVER_BACKGROUNDS[rarity]}`}>
        ...
        {rarity === FishRarity.EPIC && (
          <>
            ...
            <div className="absolute bottom-6 right-6 text-lg opacity-25">⚡</div>
          </>
        )}
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="font-semibold text-foreground text-lg">{fish.species} #{fish.id.toString()}</h3>
            <div className="flex gap-2 mt-1 flex-wrap">
              <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${RARITY_COLORS[rarityKey]}`}>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-muted-foreground font-medium">{fish.weight} kg</span>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-muted-foreground font-medium">{fish.stakedAmount} FISH</span>
          </div>
        </div>
